...
describe("gwen:physics3d — BVH error handling", () => {
  it("logs a warning and skips when the GLB file does not exist", async () => {
    ...
    try {
      ...
      expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("[gwen:physics3d]"));
  ...
  it("does not throw when build_bvh_from_glb throws (bad GLB bytes)", async () => {
    ...
    try {
      ...
      await expect(transformBvhReferences(code, "/project/src/game.ts")).resolves.toBeNull();

      expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("BVH pre-bake failed"));
    } finally {
      warnSpy.mockRestore();
