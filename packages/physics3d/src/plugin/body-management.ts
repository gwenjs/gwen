...
import type { Physics3DAPI, Physics3DBodyHandle, Physics3DBodyOptions, Physics3DEntityId } from '../types';
import type { Physics3DColliderOptions } from '../types';
import type { PluginContext } from './plugin-context';
import { toEntityIndex, vec3, quat, kindToU8, kindFromU8, parseBodyState, cloneState } from './physics3d-utils';
...
export function createBodyWasm(
  ...
  if (options.quality !== undefined) {
    const QUALITY_ITER_MAP: Record<import('../types/config').Physics3DQualityPreset, number> = {
      low: 0, medium: 0, high: 1, esport: 2,
    };
    ...
    if (ctx.backendMode === 'wasm') {
      ...
      return (
        ctx.wasmBridge!.physics3d_set_linear_velocity!(
          slot, velocity.x ?? cx, velocity.y ?? cy, velocity.z ?? cz,
        ) ?? false
      );
    }
    const state = ctx.stateByEntity.get(slot);
...
export function createSetKinematicPosition(ctx: PluginContext): Physics3DAPI['setKinematicPosition'] {
